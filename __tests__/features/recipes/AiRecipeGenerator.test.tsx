...
describe('AiRecipeGenerator', () => {
  it('désactive le bouton Générer tant que le prompt est vide', async () => {
    mockGenerate(jest.fn())
    mockCreate(jest.fn())
    const { getByTestId } = await render(<AiRecipeGenerator />)
    expect(getByTestId('generate-button').props.accessibilityState?.disabled).toBe(true)
  })

  it('active le bouton Générer une fois un prompt saisi', async () => {
    mockGenerate(jest.fn())
    mockCreate(jest.fn())
    const { getByTestId } = await render(<AiRecipeGenerator />)
    await changeText(getByTestId, 'ai-prompt-input', 'Un plat épicé avec du poulet')
  ...
  it('enregistre la recette prévisualisée au press de Enregistrer', async () => {
    mockGenerate(jest.fn((_params, { onSuccess }) => onSuccess(generatedRecipe)))
    const create = jest.fn((_recipe, { onSuccess }) => onSuccess())
    mockCreate(create)
